...
func TestIntegration_BackupCronJobs(t *testing.T) {
	ns := createTestNamespace(t)
	op := newTestOperator(t, ns)
	name := "integ-backup"

	backupProfileID := "abcdef01-2345-6789-abcd-ef0123456789"
	ruleShort := ruleShortID(backupProfileID)
	...
	cfg.Backups = []*pgswarmv1.BackupConfig{
		{
			...
			Destination: &pgswarmv1.BackupDestination{
				Type:  "local",
				Local: &pgswarmv1.LocalDestination{Size: "1Gi"},
			},
			BackupImage: "ghcr.io/pg-swarm/pg-swarm-backup:latest",
		},
	}
	...
	// CronJobs
	baseCJ := resourceName(name, "base-backup-"+ruleShort)
	logicalCJ := resourceName(name, "logical-backup-"+ruleShort)
	assertResourceExists(t, ns, "CronJob", baseCJ)
	assertResourceExists(t, ns, "CronJob", logicalCJ)

	// Backup RBAC
	backupSA := backupServiceAccountName(name)
	assertResourceExists(t, ns, "ServiceAccount", backupSA)
	assertResourceExists(t, ns, "Role", backupSA)
	assertResourceExists(t, ns, "RoleBinding", backupSA)

	// Backup PVC
	assertResourceExists(t, ns, "PersistentVolumeClaim", backupStoragePVCName(name))
}

func TestIntegration_PendingBackupTrigger(t *testing.T) {
	ns := createTestNamespace(t)
	op := newTestOperator(t, ns)
	name := "integ-trigger"

	backupProfileID := "fedcba98-7654-3210-fedc-ba9876543210"
	ruleShort := ruleShortID(backupProfileID)
	...
	ctx := context.Background()
	op.TriggerPendingBackups(ctx, ns, name)

	// Verify initial job was created
	initialJobName := resourceName(name, "base-backup-"+ruleShort) + "-initial"
	// Poll briefly — job creation is async
	deadline := time.Now().Add(15 * time.Second)
	for time.Now().Before(deadline) {
		_, err := testClient.BatchV1().Jobs(ns).Get(ctx, initialJobName, metav1.GetOptions{})
		if err == nil {
			t.Logf("initial job %s created", initialJobName)
			return
		}
		time.Sleep(2 * time.Second)
	}
	assertResourceExists(t, ns, "Job", initialJobName)
}
