...
const BUILTIN_RULES = [
  { name: 'stale-wal-recovery', pattern: 'invalid record length at .* expected at least \\d+, got 0', severity: 'critical', action: 'restart', cooldown_seconds: 60, category: 'WAL & Checkpoint', enabled: true, builtin: true },
  { name: 'checkpoint-missing', pattern: 'could not locate a valid checkpoint record', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 120, category: 'WAL & Checkpoint', enabled: true, builtin: true },
  { name: 'wal-read-error', pattern: 'could not read WAL at .* invalid record length', severity: 'critical', action: 'restart', cooldown_seconds: 60, category: 'WAL & Checkpoint', enabled: true, builtin: true },
  { name: 'wal-size-mismatch', pattern: 'WAL file .* has size \\d+, should be \\d+', severity: 'critical', action: 'restart', cooldown_seconds: 60, category: 'WAL & Checkpoint', enabled: true, builtin: true },
  { name: 'wal-prevlink-corrupt', pattern: 'record with incorrect prev-link', severity: 'critical', action: 'restart', cooldown_seconds: 60, category: 'WAL & Checkpoint', enabled: true, builtin: true },
  { name: 'timeline-not-in-history', pattern: 'requested starting point .* is not in this server.s history', severity: 'critical', action: 'rewind', cooldown_seconds: 120, category: 'Timeline & Replication', enabled: true, builtin: true },
  { name: 'ahead-of-flush', pattern: 'requested starting point .* is ahead of the WAL flush position', severity: 'critical', action: 'rewind', cooldown_seconds: 120, category: 'Timeline & Replication', enabled: true, builtin: true },
  { name: 'timeline-not-child', pattern: 'requested timeline \\d+ is not a child of this server.s history', severity: 'critical', action: 'rewind', cooldown_seconds: 120, category: 'Timeline & Replication', enabled: true, builtin: true },
  { name: 'timeline-fork', pattern: 'new timeline \\d+ is not a child of database system timeline \\d+', severity: 'critical', action: 'rewind', cooldown_seconds: 120, category: 'Timeline & Replication', enabled: true, builtin: true },
  { name: 'wal-stream-timeline', pattern: 'could not receive data from WAL stream:.*timeline', severity: 'critical', action: 'rewind', cooldown_seconds: 120, category: 'Timeline & Replication', enabled: true, builtin: true },
  { name: 'page-corruption', pattern: 'invalid page in block \\d+ of relation', severity: 'critical', action: 'event', cooldown_seconds: 300, category: 'Corruption', enabled: true, builtin: true },
  { name: 'read-block-error', pattern: 'could not read block \\d+ in file', severity: 'critical', action: 'event', cooldown_seconds: 300, category: 'Corruption', enabled: true, builtin: true },
  { name: 'disk-full', pattern: 'could not write to file.*No space left on device', severity: 'critical', action: 'event', cooldown_seconds: 60, category: 'Corruption', enabled: true, builtin: true },
  { name: 'fsync-error', pattern: 'could not fsync file', severity: 'critical', action: 'event', cooldown_seconds: 60, category: 'Corruption', enabled: true, builtin: true },
  { name: 'too-many-clients', pattern: 'FATAL:.*sorry, too many clients already', severity: 'error', action: 'event', cooldown_seconds: 30, category: 'Connections', enabled: true, builtin: true },
  { name: 'reserved-slots-full', pattern: 'FATAL:.*remaining connection slots are reserved', severity: 'error', action: 'event', cooldown_seconds: 30, category: 'Connections', enabled: true, builtin: true },
  { name: 'out-of-memory', pattern: 'ERROR:.*out of memory', severity: 'error', action: 'event', cooldown_seconds: 30, category: 'Connections', enabled: true, builtin: true },
  { name: 'shared-memory', pattern: 'out of shared memory', severity: 'error', action: 'event', cooldown_seconds: 60, category: 'Connections', enabled: true, builtin: true },
  { name: 'slot-invalidated', pattern: 'replication slot .* has been invalidated', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 300, category: 'Replication Slots', enabled: true, builtin: true },
  { name: 'wal-removed', pattern: 'requested WAL segment .* has already been removed', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 300, category: 'Replication Slots', enabled: true, builtin: true },
  { name: 'slot-missing', pattern: 'replication slot .* does not exist', severity: 'warning', action: 'event', cooldown_seconds: 120, category: 'Replication Slots', enabled: true, builtin: true },
  { name: 'walsender-timeout', pattern: 'terminating walsender process due to replication timeout', severity: 'warning', action: 'event', cooldown_seconds: 60, category: 'Replication Slots', enabled: true, builtin: true },
  { name: 'recovery-conflict', pattern: 'canceling statement due to conflict with recovery', severity: 'info', action: 'event', cooldown_seconds: 30, category: 'Replication Slots', enabled: true, builtin: true },
  { name: 'auth-failure', pattern: 'FATAL:.*password authentication failed for user', severity: 'warning', action: 'event', cooldown_seconds: 30, category: 'Authentication', enabled: true, builtin: true },
  { name: 'hba-rejection', pattern: 'FATAL:.*no pg_hba.conf entry for', severity: 'warning', action: 'event', cooldown_seconds: 30, category: 'Authentication', enabled: true, builtin: true },
  { name: 'stale-backup-label', pattern: 'FATAL:.*could not open file.*backup_label', severity: 'error', action: 'restart', cooldown_seconds: 60, category: 'Recovery', enabled: true, builtin: true },
  { name: 'wal-level-minimal', pattern: 'WAL was generated with .wal_level=minimal., cannot continue recovering', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 120, category: 'Recovery', enabled: true, builtin: true },
  { name: 'wal-dir-missing', pattern: 'FATAL:.*could not open directory.*pg_wal', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 120, category: 'Recovery', enabled: true, builtin: true },
  { name: 'version-mismatch', pattern: 'database files are incompatible with server', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 300, category: 'Recovery', enabled: true, builtin: true },
  { name: 'catalog-corruption', pattern: 'cache lookup failed for (relation|type|function|operator)', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 300, category: 'Recovery', enabled: true, builtin: true },
  { name: 'tablespace-missing', pattern: 'could not open tablespace directory', severity: 'critical', action: 'rebasebackup', cooldown_seconds: 120, category: 'Recovery', enabled: true, builtin: true },
  { name: 'recovery-complete', pattern: 'consistent recovery state reached', severity: 'info', action: 'event', cooldown_seconds: 0, category: 'Recovery', enabled: true, builtin: true },
  { name: 'streaming-started', pattern: 'started streaming WAL from primary', severity: 'info', action: 'event', cooldown_seconds: 0, category: 'Streaming', enabled: true, builtin: true },
  { name: 'primary-unreachable', pattern: 'FATAL:.*could not connect to the primary server', severity: 'warning', action: 'event', cooldown_seconds: 30, category: 'Streaming', enabled: true, builtin: true },
  { name: 'replication-terminated', pattern: 'replication terminated by primary server', severity: 'warning', action: 'event', cooldown_seconds: 60, category: 'Streaming', enabled: true, builtin: true },
  { name: 'max-walsenders', pattern: 'FATAL:.*number of requested standby connections exceeds max_wal_senders', severity: 'error', action: 'event', cooldown_seconds: 120, category: 'Streaming', enabled: true, builtin: true },
  { name: 'archive-failed', pattern: 'archive command failed with exit code', severity: 'error', action: 'event', cooldown_seconds: 60, category: 'Archive', enabled: true, builtin: true },
];
...
export const recoveryRuleSets = [
  ...
  },
  {
    ...
    rules: [
      ...
      { name: 'deadlock-alert', pattern: 'deadlock detected', severity: 'warning', action: 'event', cooldown_seconds: 30, category: 'Custom', enabled: true, builtin: false },
      { name: 'long-lock-wait', pattern: 'process \\d+ still waiting for .* lock on', severity: 'warning', action: 'event', cooldown_seconds: 60, category: 'Custom', enabled: true, builtin: false },
    ],
    created_at: ago(86400 * 10),
