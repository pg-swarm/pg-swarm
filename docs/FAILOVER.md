...
- **failover-sidecar** — Kubernetes sidecar container. Manages the leader lease, detects primary failure, promotes replicas, and runs the log watcher for pattern-based recovery.
...
| # | Scenario | Ordinal | Sentinel | PG_VERSION | Recovery | Notes |
|---|----------|---------|----------|------------|----------|-------|
| 5 | PGDATA deleted while running | 0 | Yes | No | **Wrapper yields**: writes basebackup marker, sleeps 30s. Sidecar lease expires. Replica promotes. Wrapper eventually re-basebackups from new primary. | **The critical fix.** Without the sentinel, wrapper would `initdb` and create an empty primary. |
...
| 15 | Checkpoint WAL missing | 0 | -- | Yes | `pg_resetwal -f` | Last resort: resets WAL to allow primary to start |
...
| # | Scenario | Ordinal | Sentinel | PG_VERSION | Recovery | Notes |
|---|----------|---------|----------|------------|----------|-------|
| 17 | 3+ fast crashes (<30s each) | 0 | -- | Yes | `pg_resetwal -f` | Wrapper crash-loop breaker. Resets WAL to allow start. |
| 18 | 3+ fast crashes (<30s each) | >0 | -- | Yes | `pg_basebackup` from primary | Full rebuild |
| 19 | 3+ fast crashes on primary, sidecar stops renewing lease | 0 | -- | Yes | Sidecar skips lease renewal after `crashLoopThreshold` (3). Lease expires. Replica promotes. Primary wrapper eventually recovers via `pg_resetwal`. | Sidecar's `handlePrimary()` detects crash loop and yields. |
...
8. **Log-based recovery rules are additive.** The logwatcher matches PG log lines against rules from a ConfigMap and triggers actions (`restart`, `rewind`, `rebasebackup`, `event`). Rules are severity-ordered with mutex protection — only one destructive action runs at a time.
